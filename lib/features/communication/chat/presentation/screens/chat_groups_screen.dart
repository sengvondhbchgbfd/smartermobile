...
class _ChatGroupsScreenState extends ConsumerState<ChatGroupsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.add_circle_outline_rounded, color: Color(0xFF5caeff)),
            onPressed: () => _openCreateGroup(context),
          ),
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: const TextStyle(color: Color(0xFF8a9bb0)),
                prefixIcon: const Icon(Icons.search_rounded, color: Color(0xFF8a9bb0)),
                filled: true,
          ...
          Expanded(
            child: groupsAsync.when(
              loading: () => const Center(child: CircularProgressIndicator(color: Color(0xFF5caeff))),
              error: (e, __) => ChatErrorView(error: e.toString(), onRetry: () => ref.invalidate(groupListProvider)),
              data: (groups) {
                final filtered = groups.where((g) => (g.groupName ?? '').toLowerCase().contains(_search)).toList();

                if (filtered.isEmpty) {
                  return ChatEmptyState(isSearch: _search.isNotEmpty, onCreateGroup: () => _openCreateGroup(context));
                }
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => _openCreateGroup(context),
        child: const Icon(Icons.edit_rounded, color: Color(0xFF0e1621)),
      ),
    );
  }
  ...
  void _openCreateGroup(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => const CreateGroupScreen())).then((_) => ref.invalidate(groupListProvider));
  }
}